...
function _preduceBF!(M::AbstractMatrix{T}, N::AbstractMatrix{T}, 
   ...
   if fast
      ...
      i1 = 1:n
      # [M22 M23] <- Q2'*[M22 M23]
      lmul!(F.Q',view(M,i22,j23))
      # [M12; M22] <- [M12; M22]*P2
      M[i12,j22] = M[i12,j22[jpvt]]      
      # N23 <- Q2'*N23
      lmul!(F.Q',view(N,i22,jN23))
      # N12 <- N12*P2
      N[i11,j22] = N[i11,j22[jpvt]]
      # Q <- Q*Q2
      ...
      withZ && (Z[:,j22] = Z[:,j22[jpvt]])
      # L <- Q2'*L
      ismissing(L) || lmul!(F.Q',view(L,i22,:))
      # N22 = [R2;0] 
      N22[:,:] = [ triu(N22[i1,:]); zeros(T,p,npm) ]
       # R <- R*P2
      ismissing(R) || (R[:,j22] = R[:,j22[jpvt]])
      ...
      for i = coff+n:-1:coff+1
          ...
          T <: Complex && (N[ii,ind] = conj(view(N,ii,ind)))
          H = MatrixPencils.Householder(view(N,ii,ii22),conj(τ))
      ...
      U, S, Vt = svd(view(N,i22,j22),full = true)
      tol = max(atol, rtol*S[1])
      n = count(x -> x > tol, S) 
      m = npm-n
      p = npp-n
...
function _preduce1!(n::Int, m::Int, p::Int, M::AbstractMatrix{T}, N::AbstractMatrix{T},
   ...
   if p > 0
      ...
      lmul!(F.Q',CE)
      lmul!(F.Q',EE)
      withQ && rmul!(view(Q,:,ic),F.Q)
      ismissing(L) || lmul!(F.Q',view(L,ic,:))
      D[:,:] = [ triu(F.R[1:τ,:]); zeros(T,p-τ,m)  ]
      # Step 2:
      k = 1
      for j = coff+1:coff+τ
         for ii = roff+n+k:-1:roff+1+k
...
function _preduce2!(n::Int, m::Int, p::Int, M::AbstractMatrix{T}, N::AbstractMatrix{T}, 
   ...
   if m > 0
      ...
      ismissing(R) || ( rmul!(view(R,:,jb),F.Q); reverse!(view(R,:,jb),dims=2) )
      #ismissing(R) || ( LinearAlgebra.LAPACK.ormqr!('R','N',DT,τau,view(R,:,jb)); R[:,jb] = R[:,reverse(jb)] )
      rmul!(EE,F.Q) 
      reverse!(EE,dims=2) 
      #LinearAlgebra.LAPACK.ormqr!('R','N',DT,τau,EE)
...
function _preduce3!(n::Int, m::Int, M::AbstractMatrix{T}, N::AbstractMatrix{T}, 
   ...
   if fast
      ...
      if mn > 0
         ...
         rmul!(view(M,:,jt1),F.Q)
         reverse!(view(M,:,jt1),dims=2)
         withZ && (rmul!(view(Z,:,jt1),F.Q); reverse!(view(Z,:,jt1),dims=2))
         rmul!(view(N,1:roff+mn,jt1),F.Q); reverse!(view(N,1:roff+mn,jt1),dims=2)
         ismissing(R) || (rmul!(view(R,:,jt1),F.Q); reverse!(view(R,:,jt1),dims=2)) 
         triu!(E11)      
      else
         ρ = 0
      end
...
function _preduce4!(n::Int, m::Int, p::Int, M::AbstractMatrix{T},N::AbstractMatrix{T},
   ...
   if fast
      ...
      if pn > 0
         ...
         lmul!(F.Q',view(N,it,jt1))
         ismissing(L) || lmul!(F.Q',view(L,it,:))
         triu!(E22)
      else
         ρ = 0
      end
